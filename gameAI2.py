...
def ai_control():
    """Simple AI to control the jet."""
    global player_x, player_y

    # Move towards the closest coin or life power-up
    target_x = None
    if coin:
        target_x = coin[0] + coin_width // 2
    elif life_power_up:
        target_x = life_power_up[0] + life_width // 2

    # If no target, avoid obstacles
    if target_x is None:
        for obstacle in obstacles:
            if obstacle[1] > player_y - 40:  # Consider only obstacles close to the player
                if obstacle[0] < player_x + player_width and obstacle[0] + obstacle_width > player_x:
                    if player_x > width // 2:
                        target_x = player_x - 40  # Move left if on the right
                    else:
                        target_x = player_x + 40  # Move right if on the left

    # Move horizontally towards the target
    if target_x is not None:
        if target_x < player_x + player_width // 2:
            player_x -= player_speed
        elif target_x > player_x + player_width // 2:
            player_x += player_speed

    # Ensure the player stays within bounds
    player_x = max(0, min(player_x, width - player_width))

    # Shoot bullets when obstacles are aligned
    for obstacle in obstacles:
        if abs((obstacle[0] + obstacle_width // 4) - (player_x + player_width // 4)) < 10:
            shoot_bullet()
            break
